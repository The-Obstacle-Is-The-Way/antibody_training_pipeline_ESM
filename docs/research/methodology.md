...
| **Jain** (86 clinical) | **68.60%** | 68.6% | **0pp** | ⭐ **EXACT PARITY** |
...
**Results:**
- Accuracy: **68.60%** (59/86 correct) - EXACT NOVO PARITY
- Novo target: **68.6%** (Figure S14A: [[40, 17], [10, 19]])
- Our confusion matrix: **[[40, 17], [10, 19]]** - IDENTICAL
- Gap: **0pp** - EXACT MATCH

**Analysis:** Achieved exact parity by:
1. Identifying 5 antibodies removed by Novo (murine/chimeric origin, clinical QC)
2. Applying Tier D reclassification (lebrikizumab, galiximab - chromatography flags)
3. Matching QC criteria exactly (see `novo-parity.md` for details)
...
- Accuracy: 68.60% (59/86) - EXACT NOVO PARITY
- Precision (specific): 0.80
- Recall (specific): 0.70
- F1-score (specific): 0.75
...
**Train on Boughter:**
```bash
uv run antibody-train  # Uses src/antibody_training_esm/conf/config.yaml by default
```
