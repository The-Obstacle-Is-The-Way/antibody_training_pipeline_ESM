...
class Evaluator:
    """Manages model evaluation and cross-validation."""
    ...
    def evaluate_model(
        ...
        if "precision" in metrics:
            results["precision"] = float(precision_score(y, y_pred, average="binary"))

        if "recall" in metrics:
            results["recall"] = float(recall_score(y, y_pred, average="binary"))
    ...
    def perform_cross_validation(
        ...
        cv_params["batch_size"] = config["training"].get(
        ...
        cv_results["cv_f1"] = {"mean": float(scores.mean()), "std": float(scores.std())}

        # ROC AUC
        scores = cross_val_score(cv_classifier, X, y, cv=cv, scoring="roc_auc")
        cv_results["cv_roc_auc"] = {
            "mean": float(scores.mean()),
            "std": float(scores.std()),
        }
        ...
        for metric, values in cv_results.items():
            self.logger.info(
                f"  {metric}: {values['mean']:.4f} (+/- {values['std'] * 2:.4f})"
            )
    ...
    def save_cv_results(
        ...
        self.logger.info(f"CV results saved to {cv_file}")
