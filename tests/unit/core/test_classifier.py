...
# ============================================================================
# set_params() Tests - Classifier Parameters (No Extractor Reload)
# ============================================================================


@pytest.mark.unit
def test_set_params_updates_penalty(mock_transformers_model: Any) -> None:
    """Test updating penalty parameter without recreating extractor."""
    # Arrange
    params = {
        "model_name": "facebook/esm1v_t33_650M_UR90S_1",
        "device": "cpu",
    ...
    # Act - Update penalty to l1, then back to l2
    classifier.set_params(penalty="l1")
    intermediate_extractor_id = id(classifier.embedding_extractor)
    classifier.set_params(penalty="l2")
    final_extractor_id = id(classifier.embedding_extractor)
...
@pytest.mark.unit
def test_set_params_updates_solver(mock_transformers_model: Any) -> None:
    """Test updating solver parameter without recreating extractor."""
    # Arrange
    params = {
        "model_name": "facebook/esm1v_t33_650M_UR90S_1",
        "device": "cpu",
    ...
    # Act - Update solver to liblinear, then saga
...
@pytest.mark.unit
def test_set_params_updates_class_weight(mock_transformers_model: Any) -> None:
    """Test updating class_weight parameter without recreating extractor."""
    # Arrange
    params = {
        "model_name": "facebook/esm1v_t33_650M_UR90S_1",
        "device": "cpu",
    ...
    # Act - Update class_weight to balanced, then back to None
...
@pytest.mark.unit
def test_set_params_updates_C_no_extractor_reload(
    mock_transformers_model: Any,
) -> None:
    """Test updating C alone doesn't recreate extractor."""
    # Arrange
    params = {
        "model_name": "facebook/esm1v_t33_650M_UR90S_1",
        "device": "cpu",
...
# ============================================================================
# set_params() Tests - Embedding Parameters (Extractor Reload)
# ============================================================================


@pytest.mark.unit
def test_set_params_recreates_extractor_on_batch_size_change(
    mock_transformers_model: Any, caplog: pytest.LogCaptureFixture
) -> None:
    """Test extractor is recreated when batch_size changes."""
    # Arrange
    params = {
        "model_name": "facebook/esm1v_t33_650M_UR90S_1",
        "device": "cpu",
    ...
    # Act - Update batch_size
    import logging

    caplog.set_level(logging.INFO)
    classifier.set_params(batch_size=16)

    # Assert - extractor object ID changed
    new_extractor_id = id(classifier.embedding_extractor)
    assert new_extractor_id != old_extractor_id

    # Assert - new extractor has correct batch_size
    assert classifier.batch_size == 16
    assert classifier.embedding_extractor.batch_size == 16

    # Assert - log message confirms recreation
    assert "Recreating embedding extractor" in caplog.text


@pytest.mark.unit
def test_set_params_recreates_extractor_on_model_name_change(
    mock_transformers_model: Any, caplog: pytest.LogCaptureFixture
) -> None:
    """Test extractor is recreated when model_name changes."""
    # Arrange
    params = {
        "model_name": "facebook/esm1v_t33_650M_UR90S_1",
        "device": "cpu",
    ...
    # Act - Update model_name
    import logging

    caplog.set_level(logging.INFO)
    new_model_name = "facebook/esm1v_t33_650M_UR90S_2"
    classifier.set_params(model_name=new_model_name)

    # Assert - extractor object ID changed
    new_extractor_id = id(classifier.embedding_extractor)
    assert new_extractor_id != old_extractor_id

    # Assert - new extractor has correct model_name
    assert classifier.model_name == new_model_name
...
@pytest.mark.unit
def test_set_params_recreates_extractor_on_device_change(
    mock_transformers_model: Any, caplog: pytest.LogCaptureFixture
) -> None:
    """Test extractor is recreated when device changes."""
    # Arrange
    params = {
        "model_name": "facebook/esm1v_t33_650M_UR90S_1",
        "device": "cpu",
    ...
    # Assert - new extractor uses correct device
    assert classifier.device == "cuda"
    assert classifier.embedding_extractor.device == "cuda"
...
@pytest.mark.unit
def test_set_params_recreates_extractor_on_revision_change(
    mock_transformers_model: Any, caplog: pytest.LogCaptureFixture
) -> None:
    """Test extractor is recreated when revision changes."""
    # Arrange
    params = {
        "model_name": "facebook/esm1v_t33_650M_UR90S_1",
        "device": "cpu",
    ...
    caplog.set_level(logging.INFO)
    new_revision = "some_branch"
    classifier.set_params(revision=new_revision)

    # Assert - extractor object ID changed
    new_extractor_id = id(classifier.embedding_extractor)
    assert new_extractor_id != old_extractor_id

    # Assert - new extractor uses correct revision
    assert classifier.revision == new_revision
    assert classifier.embedding_extractor.revision == new_revision
