...
# File paths
BASE_DIR = Path("/Users/ray/Desktop/CLARITY-DIGITAL-TWIN/antibody_training_pipeline_ESM")
...
def calculate_composite_risk(df):
    ...
    specific['ac_sins_norm'] = (specific['ac_sins'] - specific['ac_sins'].min()) / (specific['ac_sins'].max() - specific['ac_sins'].min())
    specific['hic_norm'] = (specific['hic'] - specific['hic'].min()) / (specific['hic'].max() - specific['hic'].min())
    specific['tm_inv_norm'] = 1 - ((specific['fab_tm'] - specific['fab_tm'].min()) / (specific['fab_tm'].max() - specific['fab_tm'].min()))
    ...
    print(f"  Risk range: {specific['risk_score'].min():.3f} - {specific['risk_score'].max():.3f}")
...
def step1_reclassify(df):
    ...
    for ab_id in TIER_A_FLIPS:
        ...
        if len(idx) > 0:
            ...
            df.loc[idx, 'reclassification_reason'] = f'Tier A: PSR={psr_val:.3f} >0.4 despite ELISA=0'
    ...
    if len(idx) > 0:
        df.loc[idx, 'label'] = 1
        df.loc[idx, 'reclassified'] = True
        df.loc[idx, 'reclassification_reason'] = 'Tier B: 61% ADA (NEJM) + aggregation + chimeric'
...
def step2_remove_top30(df):
    ...
    for i, row in enumerate(to_remove.itertuples(), 1):
        print(f"  {i:2d}. {row.id:20s} risk={row.risk_score:.3f} psr={row.psr:.3f} total_flags={row.total_flags}")

    # Mark for removal
    df['removed'] = False
    df.loc[to_remove.index, 'removed'] = True
    df.loc[to_remove.index, 'removal_reason'] = 'Top 30 by composite risk (PSR+AC-SINS+HIC+Tm)'
...
def save_outputs(df_all, df_final, removed_ids):
    ...
    audit = {
        ...
        "reclassified_ids": {
            "tier_a_psr_based": [
                {"id": row['id'], "psr": float(row['psr']), "reason": row['reclassification_reason']}
                for _, row in reclassified[reclassified['id'].isin(TIER_A_FLIPS)].iterrows()
            ],
            "tier_b_clinical": [
