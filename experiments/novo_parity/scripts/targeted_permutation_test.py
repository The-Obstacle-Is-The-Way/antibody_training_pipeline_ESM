...
# Load data
df_116 = pd.read_csv(REPO_ROOT / 'test_datasets' / 'jain' / 'jain_ELISA_ONLY_116_with_zscores.csv')
bio = pd.read_csv(REPO_ROOT / 'test_datasets' / 'jain_sd03.csv')

# Merge biophysical data
bio_clean = bio[['Name', 'Poly-Specificity Reagent (PSR) SMP Score (0-1)',
                  'Affinity-Capture Self-Interaction Nanoparticle Spectroscopy (AC-SINS) ∆λmax (nm) Average',
                  'HIC Retention Time (Min)a', 'Fab Tm by DSF (°C)']].copy()
...
def test_permutation(perm_id, config):
        ...
        if ab_id in df_work['id'].values:
            ...
            if df_work.loc[idx, 'label'] == 0:  # Only flip if currently specific
                ...
                if ab_id in config['tier_a']:
                    ...
                    df_work.loc[idx, 'reclassification_reason'] = 'Tier B: Clinical evidence'
    ...
    print(f"  Distribution: {n_spec} specific / {n_nonspec} non-specific = {n_total} total", end='')
    if n_spec == 59 and n_nonspec == 27 and n_total == 86:
        print(" ✅")
    else:
        print(f" ❌ WRONG! Expected 59/27=86")
        return None
...
if __name__ == '__main__':
    ...
    for i, r in enumerate(results_sorted[:10], 1):
        ...
        print(f"| {i} | {r['permutation_id']} | {cm_str} | {r['difference_cells']} | {r['accuracy']:.4f} | {r['description'][:50]} {mark} |")
    ...
    if exact_matches:
        ...
        print(f'Closest match: {results_sorted[0]["permutation_id"]} ({results_sorted[0]["difference_cells"]} cells off)')
