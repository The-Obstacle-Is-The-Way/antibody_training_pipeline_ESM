...
df = df_116.merge(df_sd03[['Name', 'Poly-Specificity Reagent (PSR) SMP Score (0-1)',
                             'Affinity-Capture Self-Interaction Nanoparticle Spectroscopy (AC-SINS) ∆λmax (nm) Average',
                             'HIC Retention Time (Min)a', 'Fab Tm by DSF (°C)']],
...
def apply_permutation(df, reclass_ids, removal_ids):
    ...
    assert specific_after == 89, f"Expected 89 specific after reclass, got {specific_after}"
    assert nonspec_after == 27, f"Expected 27 non-specific after reclass, got {nonspec_after}"
    ...
    assert final_nonspec == 27, f"Expected 27 non-specific in final, got {final_nonspec}"
...
def test_permutation(perm_id, reclass_strategy, removal_strategy, description):
    ...
    print(f"\n{'='*80}")
    print(f"COMPARISON TO NOVO")
    print(f"{'='*80}")
    print(f"Novo CM:  [[{NOVO_CM[0,0]:2d}, {NOVO_CM[0,1]:2d}], [{NOVO_CM[1,0]:2d}, {NOVO_CM[1,1]:2d}]]")
    print(f"Ours:     [[{cm[0,0]:2d}, {cm[0,1]:2d}], [{cm[1,0]:2d}, {cm[1,1]:2d}]]")
    print(f"Diff:     [[{cm[0,0]-NOVO_CM[0,0]:+2d}, {cm[0,1]-NOVO_CM[0,1]:+2d}], [{cm[1,0]-NOVO_CM[1,0]:+2d}, {cm[1,1]-NOVO_CM[1,1]:+2d}]]")
...
def main():
    ...
    parser.add_argument('--permutations', type=str, default='P1,P2,P3,P4,P5',
    ...
    if matches:
        ...
        for result in sorted(results, key=lambda r: np.abs(np.array(r['confusion_matrix']) - NOVO_CM).sum())[:3]:
            cm = np.array(result['confusion_matrix'])
            diff = np.abs(cm - NOVO_CM).sum()
            print(f"  {result['permutation_id']}: Difference = {diff} cells, CM = {cm.tolist()}")
