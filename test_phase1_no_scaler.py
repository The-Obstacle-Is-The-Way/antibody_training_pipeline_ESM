...
def load_cached_embeddings(cache_dir: str, dataset_name: str):
    ...
    if not cache_files:
        raise FileNotFoundError(f"No cached embeddings found for {dataset_name} in {cache_dir}")
...
def test1_no_scaler_our_hyperparams():
    ...
    """
    logger.info("=" * 80)
    logger.info("TEST 1: No StandardScaler + Our Hyperparameters (max_iter=1000, random_state=42)")
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.warning(f"Embeddings not cached, computing fresh embeddings...")
        from model import ESMEmbeddingExtractor
        embedding_extractor = ESMEmbeddingExtractor('facebook/esm1v_t33_650M_UR90S_1', 'mps', batch_size=8)
        X_train_embedded = embedding_extractor.extract_batch_embeddings(X_train_sequences)
    ...
    classifier = LogisticRegression(
        random_state=42,
        max_iter=1000,
        class_weight=None  # Boughter is balanced
    )

    logger.info("Classifier settings:")
    logger.info(f"  - max_iter: 1000")
    logger.info(f"  - random_state: 42")
    logger.info(f"  - class_weight: None")
    logger.info(f"  - StandardScaler: NO ✅")

    # 10-fold cross-validation (matching Novo)
    cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)

    logger.info("\nRunning 10-fold cross-validation...")
    cv_scores = cross_val_score(classifier, X_train_embedded, y_train, cv=cv, scoring='accuracy')
...
def test2_sklearn_defaults():
    ...
    """
    logger.info("\n\n" + "=" * 80)
    logger.info("TEST 2: No StandardScaler + sklearn Defaults (max_iter=100, no random_state)")
    logger.info("=" * 80)
    ...
    logger.info("Classifier settings:")
    logger.info(f"  - max_iter: 100 (sklearn default)")
    logger.info(f"  - random_state: None (no seed)")
    logger.info(f"  - class_weight: None (sklearn default)")
    logger.info(f"  - StandardScaler: NO ✅")
    ...
    try:
        ...
        from model import ESMEmbeddingExtractor
        embedding_extractor = ESMEmbeddingExtractor('facebook/esm1v_t33_650M_UR90S_1', 'mps', batch_size=8)
        X_jain_embedded = embedding_extractor.extract_batch_embeddings(jain_df['sequence'].tolist())
...
def compare_results(test1_results, test2_results):
    ...
    if test1_results['cv_mean'] >= 0.69:  # Within 2% of Novo's 71%
        ...
        logger.info("=" * 80)
        logger.info("StandardScaler was indeed the root cause of performance degradation.")
        logger.info("Removing StandardScaler brings CV accuracy to ~70-71% (matching Novo).")
        logger.info("\nNext steps:")
        logger.info("1. Update classifier.py to remove StandardScaler")
        logger.info("2. Simplify train.py to use simple LogisticRegression on embeddings")
        logger.info("3. Update config to use sklearn defaults (or our hyperparams if better)")
        ...
        logger.warning("=" * 80)
        logger.warning(f"CV accuracy improved to {test1_results['cv_mean']*100:.2f}%, but still below Novo's 71%.")
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("This test validates whether removing StandardScaler restores Novo's performance.")
    logger.info("")
    ...
    with open(results_file, 'w') as f:
        ...
        f.write("## Hypothesis\n\n")
        f.write("StandardScaler is hurting performance. Removing it should restore CV accuracy to ~70-71% (matching Novo).\n\n")
        f.write("## Results\n\n")
        f.write(f"### Test 1: No StandardScaler + Our Hyperparameters (max_iter=1000, random_state=42)\n")
        f.write(f"- CV Accuracy: {test1_results['cv_mean']*100:.2f}% (+/- {test1_results['cv_std']*2*100:.2f}%)\n")
        f.write(f"- Training Accuracy: {test1_results['train_acc']*100:.2f}%\n")
        f.write(f"- Jain Test Accuracy: {test1_results['jain_acc']*100:.2f}%\n\n")

        f.write(f"### Test 2: No StandardScaler + sklearn Defaults (max_iter=100, no random_state)\n")
        f.write(f"- CV Accuracy: {test2_results['cv_mean']*100:.2f}% (+/- {test2_results['cv_std']*2*100:.2f}%)\n")
        ...
        f.write("|---------------|-------------|----------------|---------------|\n")
        f.write(f"| Baseline (with StandardScaler) | 63.88% | 95.62% | 55.32% |\n")
        f.write(f"| Test 1 (no scaler, max_iter=1000) | {test1_results['cv_mean']*100:.2f}% | {test1_results['train_acc']*100:.2f}% | {test1_results['jain_acc']*100:.2f}% |\n")
        f.write(f"| Test 2 (no scaler, sklearn defaults) | {test2_results['cv_mean']*100:.2f}% | {test2_results['train_acc']*100:.2f}% | {test2_results['jain_acc']*100:.2f}% |\n")
        f.write(f"| **Novo Benchmark (2025)** | **71.00%** | ~75-80% | **69.00%** |\n\n")

        f.write("## Conclusion\n\n")
        if hypothesis_confirmed:
            f.write("✅ **HYPOTHESIS CONFIRMED**\n\n")
            f.write("StandardScaler was the root cause. Removing it restores performance to Novo's benchmarks.\n\n")
            f.write("**Next steps:**\n")
            f.write("1. Remove StandardScaler from classifier.py\n")
            f.write("2. Simplify architecture to match Novo (embeddings → LogisticRegression)\n")
            ...
            f.write("Removing StandardScaler improved performance, but we're still below Novo's benchmarks.\n")
            f.write("Additional investigations needed (see console output for details).\n")
