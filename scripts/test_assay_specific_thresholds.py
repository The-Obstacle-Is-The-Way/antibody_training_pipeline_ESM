...
def test_with_assay_type(model, test_file, dataset_name, assay_type, target_cm):
    ...
    # Extract embeddings
    print(f"  Extracting ESM-1v embeddings...")
    ...
    if diff < 5:
        print(f"  ✓ Close match to Novo!")
    elif diff < 10:
        print(f"  ~ Reasonable match to Novo")
    else:
        print(f"  ✗ Significant difference from Novo")
...
def main():
    ...
    print(f"\nNovo benchmarks:")
    print(f"  Jain (ELISA):   68.6% accuracy")
    print(f"  Shehata (PSR):  58.8% accuracy")
