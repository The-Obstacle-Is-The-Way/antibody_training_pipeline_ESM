...
class HyperparameterSweep:
    ...
    def test_hyperparameters(self, C, penalty, solver, class_weight="balanced"):
        ...
        try:
            ...
            results = {
                ...
                "overfitting_gap": cv_results["train_accuracy"].mean()
                - cv_results["test_accuracy"].mean(),
            }

            logger.info(
                f"CV Accuracy: {results['cv_accuracy_mean']:.4f} ± {results['cv_accuracy_std']:.4f}"
            )
            logger.info(
                f"Train Accuracy: {results['train_accuracy_mean']:.4f} ± {results['train_accuracy_std']:.4f}"
            )
            logger.info(f"Overfitting Gap: {results['overfitting_gap']:.4f}")
            logger.info(
                f"F1: {results['cv_f1_mean']:.4f}, ROC-AUC: {results['cv_roc_auc_mean']:.4f}"
            )
    ...
    def run_sweep(self, output_dir="hyperparameter_sweep_results"):
        ...
        for i, params in enumerate(sweep_grid, 1):
            ...
            results_df.to_csv(
                f"{output_dir}/sweep_results_{timestamp}.csv", index=False
            )
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        results_df.to_csv(
            f"{output_dir}/final_sweep_results_{timestamp}.csv", index=False
        )
