...
def analyze_dataset_probabilities(model, test_file, dataset_name, target_cm=None):
    ...
    logger.info("  Specific antibodies (label=0):")
    ...
    logger.info("  Non-specific antibodies (label=1):")
    ...
    if target_cm is not None:
        ...
        logger.info(
            f"    Difference from target: {best_diff} (sum of absolute differences)"
        )
...
def main():
    ...
    if abs(jain_threshold - shehata_threshold) < 0.01:
        ...
        logger.info("  → We get our current results (not Novo's)")
        ...
        logger.info("  → We would lose Jain parity with Novo!")

        logger.info("\n" + "=" * 60)
        logger.info("CONCLUSION:")
        logger.info("=" * 60)
        logger.info("The difference in optimal thresholds reflects the fundamental")
        logger.info("difference between ELISA (Jain) and PSR (Shehata) assays.")
        logger.info("")
        logger.info("Novo's paper states (Section 2.7):")
        logger.info("  'Antibodies characterised by the PSR assay appear to be on")
        logger.info("   a different non-specificity spectrum than that from the")
        logger.info("   non-specificity ELISA assay'")
        logger.info("")
        logger.info("Our model was trained on ELISA data (Boughter), so it's")
        logger.info("optimized for ELISA thresholds. The PSR assay measures a")
        logger.info("different 'spectrum' of non-specificity.")
        logger.info("")
        logger.info("To implement dataset-specific thresholds, we would need to:")
        logger.info("1. Detect which assay type the input data uses (ELISA vs PSR)")
        logger.info("2. Apply the corresponding threshold:")
