...
def load_data():
    ...
    print(f"  Distribution: {(df['label']==0).sum()} specific / {(df['label']==1).sum()} non-specific\n")
    return df
...
def calculate_sequence_features(df):
    ...
    def calc_charge(seq):
        if pd.isna(seq):
            return np.nan
        pos = seq.count('K') + seq.count('R') + seq.count('H')  # His ~50% protonated at pH 7.4
    ...
    print(f"  VH length: mean={df['vh_length'].mean():.1f}, std={df['vh_length'].std():.1f}")
    print(f"  VL length: mean={df['vl_length'].mean():.1f}, std={df['vl_length'].std():.1f}")
    print(f"  VH charge: mean={df['vh_charge'].mean():.1f}, std={df['vh_charge'].std():.1f}\n")
...
def calculate_zscores(df):
    ...
    df['any_outlier'] = (
        df['vh_length_outlier'] |
        df['vl_length_outlier'] |
        df['charge_outlier']
    )
...
def flag_known_issues(df):
    ...
    chimeric = ['muromonab', 'cetuximab', 'basiliximab', 'infliximab',
                'rituximab', 'trastuzumab', 'abciximab', 'daclizumab',
                'gemtuzumab', 'alemtuzumab', 'ibritumomab', 'tositumomab',
                'bevacizumab', 'panitumumab', 'girentuximab']

    # Known discontinued/failed antibodies
    discontinued = ['muromonab', 'girentuximab', 'tabalumab', 'abituzumab',
                   'figitumumab', 'dalotuzumab', 'ganitumab', 'robatumumab',
                   'gemtuzumab', 'zanolimumab', 'epratuzumab', 'farletuzumab',
                   'lucatumumab', 'mapatumumab', 'tigatuzumab', 'lexatumumab']
...
def identify_qc_candidates(df):
    ...
    print(f"  Need to remove: 30 antibodies (116 → 86)")
    ...
    cols = ['id', 'label', 'elisa_flags', 'vh_length', 'vh_length_zscore',
            'vl_length', 'vl_length_zscore', 'total_charge', 'total_charge_zscore',
            'chimeric', 'discontinued', 'withdrawn']
    ...
    for idx, row in candidates.iterrows():
        ...
        print(f"{row['id']:20s} | {label_str:8s} | ELISA={row['elisa_flags']} | "
...
def analyze_by_label(df, candidates):
    ...
    print(f"Target distribution (86): 59 specific / 27 non-specific")
    print(f"Need to remove: 35 specific / -5 non-specific (?!)")
    print(f"\n⚠️  PROBLEM: Novo has MORE non-specific (27 vs 22)")
    print(f"    This suggests they RECLASSIFIED 5 antibodies from specific → non-specific")
    print(f"    OR used different ELISA thresholding")
...
def main():
    ...
    print(f"   This suggests threshold change OR reclassification")
    print("=" * 80)
