...
def check_cdr_lengths(df):
    ...
    if 'cdr1_aa_H' in df.columns:
        ...
        if h1_too_short.sum() > 0:
            issues.append(('CDR-H1 too short (<8)', df[h1_too_short], h1_too_short.sum()))
    ...
    if 'cdr2_aa_H' in df.columns:
        ...
        if h2_too_short.sum() > 0:
            issues.append(('CDR-H2 too short (<6)', df[h2_too_short], h2_too_short.sum()))
    ...
    if 'cdr3_aa_H' in df.columns:
        ...
        if h3_too_short.sum() > 0:
            issues.append(('CDR-H3 too short (<4)', df[h3_too_short], h3_too_short.sum()))
...
def audit_dataset(file_path, dataset_name):
    ...
    if gap_count > 0:
        ...
        print(f"   ✅ No gap characters found")
    ...
    if x_count > 0:
        ...
        for idx, row in x_df.head(10).iterrows():
            seq = str(row[seq_col])
            x_positions = [i for i, c in enumerate(seq) if c == 'X']
            print(f"      Row {row[id_col]}: X at positions {x_positions[:5]}... (seq preview: {seq[:50]}...)")
    else:
        print(f"   ✅ No X amino acids found")
    ...
    if empty_count > 0:
        ...
        print(f"   ✅ No empty sequences found")
    ...
    if too_short_count > 0:
        ...
        print(f"   ✅ No sequences too short (<95 aa)")
    ...
    if too_long_count > 0:
        ...
        print(f"   ✅ No sequences too long (>500 aa)")
    ...
    if homo_count > 0:
        ...
        for idx, row in homo_df.head(3).iterrows():
            seq = str(row[seq_col])
            for aa in STANDARD_AA:
                if aa * 7 in seq:
                    print(f"      Row {row[id_col]}: contains {aa}×7+")
    else:
        print(f"   ✅ No long homopolymers found")
    ...
    if cdr_issues:
        for issue_type, issue_df, issue_count in cdr_issues:
            ...
            print(f"   Examples (row indices): {issue_df[id_col].head(3).tolist() if id_col in issue_df.columns else 'N/A'}")
    else:
        print(f"   ✅ All CDR lengths look normal")
    ...
    if len(flagged_sequences) > 0:
        ...
        print(f"If Novo filtered these, their training set would be:")
...
if __name__ == "__main__":
    ...
    training_results = audit_dataset(TRAINING_FILE, "Boughter Training Set (914 sequences)")
    ...
    print(f"   Flagged: {len(training_results['flagged_sequences'])} ({len(training_results['flagged_sequences'])/training_results['total_sequences']*100:.1f}%)")
    print(f"\nFull Set (1,065 sequences):")
    print(f"   Flagged: {len(full_results['flagged_sequences'])} ({len(full_results['flagged_sequences'])/full_results['total_sequences']*100:.1f}%)")
    ...
    if len(training_results['flagged_sequences']) == 0:
        print(f"✅ Our training set is CLEAN - no QC issues found")
        print(f"✅ The 3.5% gap is NOT due to missing QC filtering")
        print(f"✅ Gap is likely due to hyperparameters, random seed, or ESM embedding differences")
    else:
        print(f"❌ Found {len(training_results['flagged_sequences'])} sequences with potential QC issues")
        print(f"❌ Novo may have filtered these, explaining part of the accuracy gap")
        print(f"❌ Recommend additional QC filtering to match Novo's methodology")
