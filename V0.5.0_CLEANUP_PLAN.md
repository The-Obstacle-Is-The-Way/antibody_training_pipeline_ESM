...
**Status**: 📋 PLANNING - Awaiting Senior Review
**Created**: 2025-11-15
...
## 🔴 **CRITICAL: ASSUMED REPOSITORY LAYOUT (POST–PHASE 5)**

**WARNING**: This plan now assumes the Phase 5 reorganization has been completed. All path references
expect the consolidated `experiments/` hierarchy:

- `configs/config.yaml` still lives at repo root until this plan deletes it.
- `experiments/cache/` contains embedding caches (moved from the former `embeddings_cache/` root dir).
- `experiments/checkpoints/` contains tracked model artifacts (moved from the former `models/` dir).

If your checkout still has `embeddings_cache/` or `models/` at repo root, **run Phase 5 first** (see
`REPOSITORY_REORGANIZATION_PLAN.md`). Running v0.5.0 cleanup against the pre-Phase 5 layout will miss
files and reintroduce root clutter.

**Reality Check**:
- This plan references `configs/config.yaml` (line 32, 53, 60) and DELETES it.
- This plan references `experiments/cache/` (lines 21, 87) for embedding caches.
- This plan references `experiments/checkpoints/` (lines 12, 93) for model outputs.

**Execution Rule**: ✅ **Phase 5 must be complete before starting v0.5.0 cleanup**. If Phase 5 has not
run, STOP and execute it; if it has already run, this document is ready to follow as-is.
...
**Scope**:
- ✅ **Problem 1**: Remove `configs/config.yaml` and legacy `train_model()` function (2-3h)
- ✅ **Problem 2**: Archive stale documentation with configs/ references (1-2h)
- ✅ **Problem 3**: Final validation and testing (1h)
- ❎ **Problem 4**: Experiments directory reorganization (handled by Phase 5, no action in this plan)
...
**Documentation References**: 18 files total (16 existing docs + POST_MIGRATION + this plan)
- `docs/archive/`: 7 files
- `experiments/strict_qc_2025-11-04/`: 6 files (⚠️ **only present on the `archive` branch after commit 11f1ba5**)
- `docs_burner/`: 2 files
- `docs/`: 1 file (ESM1V_ENSEMBLING_INVESTIGATION.md)
- Root: 2 files (POST_MIGRATION_VALIDATION_PLAN.md + this plan)

### Path Centralization Status ✅ ALREADY EXCELLENT

**Dataset Paths**: ✅ Centralized in `src/antibody_training_esm/datasets/default_paths.py`
```python
...
**Embeddings Cache**: ✅ Centralized in `src/antibody_training_esm/conf/config_schema.py`
```python
embeddings_cache_dir: str = "./experiments/cache"
```

**Model Output Paths**: ✅ Centralized in `src/antibody_training_esm/core/directory_utils.py`
```python
def get_hierarchical_model_dir(
    model_name: str,
    classifier_type: str,
    base_dir: Path = Path("experiments/checkpoints"),
) -> Path:
    # Returns: {base_dir}/{model_shortname}/{classifier_type}/
```
...
- **Deprecation warnings**: ✅ Already in place for `train_model()`
...
```python
# src/antibody_training_esm/core/__init__.py:12-13
from antibody_training_esm.core.trainer import load_model_from_npz, train_model

# src/antibody_training_esm/core/__init__.py:18
__all__ = [
    "train_model",  # ← REMOVE THIS
]
```

**Migration Status**:
- ✅ Deprecation warning added (emits `DeprecationWarning` since v0.4.0)
- ✅ Modern API available: `train_pipeline(cfg)` with Hydra
- ✅ All production code uses Hydra (antibody-train CLI)
- ⚠️ 2 preprocessing scripts still use old API
- ⚠️ 2 e2e tests use old API
...
#### Step 3: Remove Legacy Code (20 min)

**Step 3a: Remove public API export** (5 min):
```python
# Edit src/antibody_training_esm/core/__init__.py
...
**Verification**:
```bash
grep "train_model" src/antibody_training_esm/core/__init__.py
# Should return empty (no matches)
```

**Step 3b: Delete function from trainer.py** (5 min):
```bash
# Verify current line numbers
grep -n "def train_model" src/antibody_training_esm/core/trainer.py

# Use Edit tool to remove lines 793-850 (58 lines)
```

**Step 3c: Delete directory** (5 min):
```bash
git rm -r configs/
```
...
**Verification**:
```bash
# 1. No imports of deleted function
grep -r "from.*trainer import train_model" src/ tests/
# Should return empty

# 2. No train_model in public API
grep "train_model" src/antibody_training_esm/core/__init__.py
# Should return empty
...
# 4. No train_model function definition
grep -n "def train_model" src/antibody_training_esm/core/trainer.py
# Should return empty
...
**Old API (REMOVED)**:
```python
# Option 1: Direct import (REMOVED)
from antibody_training_esm.core.trainer import train_model
results = train_model("configs/config.yaml")

# Option 2: Public API import (ALSO REMOVED)
from antibody_training_esm.core import train_model
results = train_model("configs/config.yaml")
```
...
- [ ] Update config paths: `configs/config.yaml` → `src/antibody_training_esm/conf/config.yaml`
...
- [ ] `train_model` removed from `__init__.py` public API exports
- [ ] Zero `train_model` in `src/antibody_training_esm/core/__init__.py`
...
- [ ] Zero definitions of `train_model` (grep `def train_model` returns empty)
...
**Documentation with configs/ references**: 18 files total
- `docs/archive/`: 7 files (migration plans, audit reports)
- `experiments/strict_qc_2025-11-04/`: 6 files (experiment docs + READMEs, **archive branch only**)
- `docs_burner/`: 2 files (REPOSITORY_CLEANUP_PLAN.md, TRAIN_DATASETS_CONSOLIDATION_PLAN.md)
- `docs/`: 1 file (ESM1V_ENSEMBLING_INVESTIGATION.md)
- Root: 2 files (POST_MIGRATION_VALIDATION_PLAN.md, this plan)
...
```markdown
> **⚠️ HISTORICAL DOCUMENT**: This document references legacy `configs/config.yaml` which was removed in v0.5.0. For current documentation, see `docs/`.
```
...
⚠️ **Strict QC directories were removed from `dev` in commit 11f1ba5.** The files now live only on the `archive`
branch. To edit/read them you MUST checkout the archive branch:
```bash
git checkout archive
# ... perform documentation updates under experiments/benchmarks/strict_qc/*
git checkout dev
```

**Strategy**: Leave experiments docs untouched on `dev` (they no longer exist). If additional historical context
is needed, add a README on the archive branch at
`experiments/benchmarks/strict_qc/docs/README.md` with the following contents:
```markdown
# Strict QC Experiment Documentation (2025-11-04)

**Status**: ARCHIVED - Historical experiment documentation (available only on the `archive` branch)**

This directory contains documentation from the strict QC filtering experiment conducted on 2025-11-04.
References to `configs/config.yaml` are preserved as historical context.

**For current documentation**, see the main `docs/` directory on `dev`.
```

**Similar READMEs (only if the directories exist on the `archive` branch)**:
- `experiments/benchmarks/archive/hyperparameter_sweeps_2025-11-02/`
...
- [ ] Experiment docs on the `archive` branch have README.md clarifying archived status (no-op on `dev`)
...
# 4. All imports valid
uv run python -c "from antibody_training_esm.core.trainer import train_pipeline; print('✅ Imports valid')"
# Expected: ✅ Imports valid
...
## Problem 4: Experiments Directory Reorganization (Handled by Phase 5)

Phase 5 already migrated all artifacts into the canonical `experiments/` hierarchy (runs,
checkpoints, cache, benchmarks). No work remains here for v0.5.0; leave this section as a pointer to
`REPOSITORY_REORGANIZATION_PLAN.md` if rollback guidance is ever needed.
...
| Remove legacy code | 20 min | Tests updated | P0 |
...
**Question 1**: Should we keep `train_model()` as deprecated stub for one more release (v0.5.0 → v0.6.0 removal)?
- **Pro**: Gentler migration path, clearer deprecation period
- **Con**: Delays cleanup, maintains dual API surface

**Question 2**: How do we coordinate with Phase 5 experiments reorganization?
- **Resolution**: Phase 5 owns the directory move. v0.5.0 assumes that work is already complete.

**Question 3**: Are there any other legacy patterns to remove in v0.5.0?
- **Audit shows**: Path centralization ✅ complete, magic numbers ✅ clean, TODO/FIXME ✅ none
...
**Plan Status**: ✅ COMPLETE - Ready for senior review and approval
