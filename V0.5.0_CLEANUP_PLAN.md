...
## 🔴 **CRITICAL: SEQUENCING CONSTRAINT WITH PHASE 5**

**WARNING**: This plan assumes current directory structure (`configs/`, `embeddings_cache/`, `models/` at root). If Phase 5 executes first, ALL path references in this plan are invalid.

**Reality Check**:
- This plan references `configs/config.yaml` (line 32, 53, 60) and DELETES it
- This plan references `embeddings_cache/` at root (line 12, 87)
- This plan references `models/` at root (line 12, 93)

**Phase 5 Plan** (`REPOSITORY_REORGANIZATION_PLAN.md`):
- Updates `configs/config.yaml` paths (Step 2.2:223-227, Step 2.3:256, Step 4.3:420) — NOTE: May be deleted by v0.5.0 first
- Moves `embeddings_cache/` → `experiments/cache/`
- Moves `models/` → `experiments/checkpoints/`

**Key Conflict**:
- V0.5.0 deletes `configs/config.yaml` (this plan's Step 2)
- Phase 5 updates `configs/config.yaml` (with fallback "if still exists after v0.5.0")
- **No actual conflict**: Phase 5 handles both cases (deleted OR update if exists)

**Sequencing Options**:
1. **Option A (STRONGLY RECOMMENDED)**: Execute v0.5.0 cleanup FIRST, then Phase 5 repository reorganization
   - ✅ No plan conflicts (Phase 5 handles deleted configs/config.yaml)
   - ✅ No path rewrites needed
   - ✅ Cleaner execution
   - ⚠️ Requires two separate commits

2. **Option B (REQUIRES PATH REWRITES)**: Execute Phase 5 reorganization FIRST, then revise this plan
   - ❌ Must rewrite ALL path references in this plan (embeddings_cache/, models/)
   - ✅ configs/config.yaml deletion still valid (Phase 5 doesn't require it)
   - ⚠️ More complex but feasible

**DECISION REQUIRED**: ⚠️ **STRONGLY RECOMMEND EXECUTING V0.5.0 BEFORE PHASE 5** ⚠️

If Phase 5 has already executed, STOP and revise this plan entirely before proceeding.

See `REPOSITORY_REORGANIZATION_PLAN.md` for Phase 5 details.
...
- ⚠️ **Problem 4**: Experiments directory reorganization (OPTIONAL - 1h)
...
├── README.md
├── archive/
│   └── hyperparameter_sweeps_2025-11-02/ (22 files)
├── hyperparameter_sweeps/ (empty or minimal)
├── novo_parity/
...
1. **Consolidate archived experiments** under `experiments/archive/`
   - Move `strict_qc_2025-11-04/` → `experiments/archive/strict_qc_2025-11-04/`
