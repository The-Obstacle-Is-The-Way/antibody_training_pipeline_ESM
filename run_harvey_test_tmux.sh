#!/bin/bash
# Harvey Test Runner with Full Logging
# Date: November 3, 2025

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="harvey_test_${TIMESTAMP}.log"
SESSION_NAME="harvey_test"

echo "=== HARVEY DATASET TEST RUNNER ==="
echo "Timestamp: ${TIMESTAMP}"
echo "Log file: ${LOG_FILE}"
echo "Tmux session: ${SESSION_NAME}"
echo ""
echo "This will run the Harvey test with:"
echo "- PSR threshold: 0.5495 (same as Shehata)"
echo "- Full logging to: ${LOG_FILE}"
echo "- Tmux session: ${SESSION_NAME}"
echo ""

# Create Python test script
cat > test_harvey_only.py << 'PYTHON_EOF'
"""
...
echo "Created test_harvey_only.py"
echo ""

# Start tmux session with logging
tmux new-session -d -s "${SESSION_NAME}" \
    "python3 test_harvey_only.py 2>&1 | tee ${LOG_FILE}"
